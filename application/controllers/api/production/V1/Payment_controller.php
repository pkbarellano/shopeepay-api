...
class Payment_controller extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $this->client = new SPProductionClient();

        $this->load->model('production/V1/Payment_model');
        $this->load->model('production/V1/TransactionCheck_model');
        $this->load->model('production/V1/POSTransaction_model');
    }
    ...
    private function _SPClientPayment($requestID = "", $payload = [])
    {
        ...
        $body = [
            ...
            'merchant_ext_id' => (string)$this->defaultParameterProduction->merchantExtId,
            'store_ext_id' => (string)$this->defaultParameterProduction->storeExtId,
            ...
            'currency' => (string)$this->defaultParameterProduction->currency,
            'additional_info' => (string)$payload['additionalInfo']
        ];

        return $this->client->sendRequest(
            $this->appConfigProduction->paymentURL,
            $this->appConfigProduction->clientId,
    ...
    private function _SPClientTransactionCheck($requestID = "", $transactionType = 13, $payload = [])
    {
        ...
        return $this->client->sendRequest(
            $this->appConfigProduction->transactionCheckURL,
            $this->appConfigProduction->clientId,
            $this->appConfigProduction->secret,
            $body,
            0,
    ...
    private function _transactionCheckFallback($referenceID = "", $payload = [])
    {
        ...
        if ($referenceID !== null) {
            ...
            $this->createSysLog('production', $referenceID, $_SERVER['REQUEST_METHOD'], __CLASS__, __METHOD__, 'TRANSACTION_CHECK', $payload['terminalID'], $payload['terminalIPAddress'], '', $resp['responseInfo']);
